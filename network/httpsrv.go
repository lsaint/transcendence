...
func (this *HttpServer) Start() {
	for _, url := range this.urls {
		http.HandleFunc(url, this.onReq)
	}
...
func (this *HttpServer) onReq(w http.ResponseWriter, r *http.Request) {
	...
	select {
	case this.reqChan <- &HttpReq{string(recv_post), ret_chan, r.URL.Path}:
		ret = <-ret_chan
